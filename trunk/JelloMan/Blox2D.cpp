...
void TextBox::Tick()
{
	...
	if (m_bHasFocus)
	{
		...
		if (CONTROLS->IsKeyPressed(VK_DECIMAL) || CONTROLS->IsKeyPressed('.'))
			stream << _T(".");
		...
		if (CONTROLS->IsKeyDown(VK_SHIFT) || CONTROLS->IsKeyDown(VK_LSHIFT))
		{
			for (int i = 48; i < 58; i++)
			{  
				...
				{
					tstringstream strm;
					strm << m_NewText << numKeys[i - 48].c_str();
					m_NewText = strm.str();
				}           
			}
		}
		...
		{  
			...
			{
				tstringstream strm;
				strm << m_NewText << numKeys[i - 60].c_str();
				m_NewText = strm.str();
			}           
		}
        ...
        {  
            if (GetAsyncKeyState(i) == -32767)
            {
                ...
                GetKeyNameText(uScanCode << 16, key, MAX_PATH);
				/*tstring test = key;
